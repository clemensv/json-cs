# encoding: utf-8
"""
schema_core_validator.py

A validation tool for the experimental JSON Schema Core specification
published by C. Vasters (Microsoft) in February 2025.
This version optionally supports the JSON Schema Import extension via the
--allowimport flag and allows passing a mapping of URI to filenames via the
--importmap option.

Usage:
    python schema_core_validator.py [--metaschema] [--allowimport] [--importmap URI=filename ...] <path_to_json_file>
...
class JSONSchemaCoreValidator:
    """
    Validates JSON Schema Core documents for conformance with the specification.
    Provides error messages annotated with estimated line and column numbers.
    Optionally supports the JSON Schema Import extension if allow_import is True.
    """
    ...
    def validate(self, doc, source_text=None):
        """
        Main entry point for validating a JSON Schema Core document.
            ...
            for key in list(obj.keys()):
                if key in ("$import", "$importdefs"):
                    if not self.allow_import:
                        self._err(f"Import keyword '{key}' encountered but allow_import not enabled.", f"{path}/{key}")
                        continue
                    uri = obj[key]
                    if not isinstance(uri, str):
                        self._err(f"Import keyword '{key}' value must be a string URI.", f"{path}/{key}")
                        continue
                    if not self.ABSOLUTE_URI_REGEX.search(uri):
                        self._err(f"Import keyword '{key}' value must be an absolute URI.", f"{path}/{key}")
                        continue
...
def validate_json_schema_core(schema_document, source_text=None, allow_dollar=False, allow_import=False, import_map=None):
    """
    Validates the provided schema_document dict against the JSON Schema Core specification.
    ...
    """
    validator = JSONSchemaCoreValidator(allow_dollar=allow_dollar, allow_import=allow_import, import_map=import_map)
...
def main():
    ...
    if len(args) < 1:
        print("Usage: python schema_core_validator.py [--metaschema] [--allowimport] [--importmap URI=filename ...] <path_to_json_file>")
        sys.exit(1)
    file_path = args[0]
    try:
        ...
        sys.exit(1)
    errors = validate_json_schema_core(data, source_text, allow_dollar=allow_dollar, allow_import=allow_import, import_map=import_map)
    if errors:
